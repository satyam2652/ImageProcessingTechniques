...
Dila_image = np.zeros(ip_ex.shape,np.uint8)
Boundary_image = np.zeros(ip_ex.shape,np.uint8)
Eros_image = np.zeros(ip_ex.shape,np.uint8)
Boundary_image2 = np.zeros(ip_ex.shape,np.uint8)

Dil=Dilation(Inv_ex,se1,Dila_image)
for p in range (Inv_ex.shape[0]):
    for q in range (Inv_ex.shape[1]):
        Boundary_image[p,q]=Dil[p,q]-Inv_ex[p,q]
cv.imshow("Result of Boundary Extractione 1",Boundary_image)        

Ero=Erosion(Inv_ex,se1,Eros_image)
for p in range (Inv_ex.shape[0]):
    for q in range (Inv_ex.shape[1]):
        Boundary_image2[p,q]=Inv_ex[p,q]-Ero[p,q]
cv.imshow("Result of Boundary Extraction 2",Boundary_image2)        
